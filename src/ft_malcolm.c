...
int create_socket(t_session *session, char *interface) {
    ...
    if (ifindex == 0) {
        ft_putstr_fd("ft_malcolm: if_nametoindex failed\n", 2);
        return (-1);
    }
    ...
    session->sockfd =
        socket(session->sll.sll_family, SOCK_RAW, session->sll.sll_protocol);
    if (session->sockfd < 0) {
...
char *get_interface() {
    ...
    if (getifaddrs(&ifaddr) == -1) {
        ft_putstr_fd("ft_malcolm: getifaddrs failed\n", 2);
        return (NULL);
    }
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        if (ifa->ifa_addr->sa_family == AF_PACKET) {
            ...
            if ((flags & IFF_UP) && (flags & IFF_RUNNING) &&
                    !(flags & IFF_LOOPBACK)) {
                ft_strncpy(iface_name, ifa->ifa_name, IF_NAMESIZE - 1);
...
int work(t_options *opts) {
    ...
    while (g_running) {
        ...
        int bytes = recvfrom(session.sockfd, buffer, sizeof(buffer), 0,
        ...
        if (ntohs(eth->h_proto) == ETH_P_ARP) {
            ...
            if (ntohs(arp->ea_hdr.ar_op) == ARPOP_REQUEST) {
                ...
                if (ft_memcmp(arp->arp_spa, &opts->target_ip, 4) == 0 &&
                    ...
                    if (send_arp_reply(&session, opts) < 0) {
                        close(session.sockfd);
                        return -1;
                    }
...
int fill_opts(char **av, t_options *opts) {
    ...
    if (!opts->src_ip_str || !opts->src_mac_str || !opts->target_mac_str ||
            !opts->target_ip_str) {
